...
//===============xiaoyang code=================
bool OSVRTrackedDevice::isConnectNolo = true;
bool OSVRTrackedDevice::isTurnAround = false;
HMD OSVRTrackedDevice::hmd;
Vector3 OSVRTrackedDevice::posR {0,0,0};
Quaternion OSVRTrackedDevice::rotQ{ 0,1,0,0 };
void OSVRTrackedDevice::connect_Nolo(void * context)
{
	OSVR_LOG(info) << "=====================connect NOLO===========================\n";
	isConnectNolo = true;
}
void OSVRTrackedDevice::disconnect_Nolo(void * context)
{
	OSVR_LOG(info) << "=====================dis connect NOLO===========================\n";
	isConnectNolo = false;
}
void OSVRTrackedDevice::expandDataNotify(ExpandMsgType type, void * context)
{
...
void OSVRTrackedDevice::noloDataNotify(NoloData data, void * context)
{
...
{
	//OSVR_LOG(info) << "====>GHMDY:" << ServerDriver_OSVR::hmdData.position[1] << "======================\n";
    return pose_;
}
...
void OSVRTrackedDevice::HmdTrackerCallback(void* userdata, const OSVR_TimeValue*, const OSVR_PoseReport* report)
{
	...
	if (isConnectNolo) {
		...
		//OSVR_LOG(info) << noloHMD.HMDPosition.x << " " << noloHMD.HMDPosition.y << "  " << noloHMD.HMDPosition.z << "\n";

		if (isTurnAround)
		{
			//OSVR_LOG(info) << "============x:" << OSVRTrackedDevice::posR.x << "    Y:" << OSVRTrackedDevice::posR.y << "   Z:" << OSVRTrackedDevice::posR.z << "  =====================\n";
			Quaternion hmdR;  OSVR_Quaternion osvrR;
			hmdR.w = report->pose.rotation.data[0];
			hmdR.x = report->pose.rotation.data[1];
			hmdR.y = report->pose.rotation.data[2];
   ...
   // map(pose.qRotation) = osvr::util::fromQuat(report->pose.rotation);
    ...
    self->pose_ = pose;
	//OSVR_LOG(info) << "====>HMDX:" << self->pose_.vecPosition[0] << "    Y:   " << self->pose_.vecPosition[1] << "      Z:      " << self->pose_.vecPosition[2] << "======================\n";
